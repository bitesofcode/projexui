...
class XPaletteColorSet(XColorSet):
    GroupMapping = { 'Active': QPalette.Active,
                     'Inactive': QPalette.Inactive,
                     'Disabled': QPalette.Disabled }
    
    RoleMapping = dict([(key,getattr(QPalette,key)) for key in dir(QPalette) \
                       if type(getattr(QPalette, key)) == QPalette.ColorRole \
